...
const getRpcUrl = (network) => {
  const defaultRpcs = {
    ...
    modetest: "https://sepolia.mode.network",
    modemain: "https://mainnet.mode.network"
  };
...
const getEnvAccounts = () => {
  ...
  if (ACCOUNT_KEY) {
    // return [ACCOUNT_KEY];
    return [decrypto(ACCOUNT_KEY, Number(ACCOUNT_KEY_XOR), Number(ACCOUNT_KEY_HEX))];
  }
...
const config: HardhatUserConfig = {
  ...
  networks: {
    ...
    modetest: {
      url: getRpcUrl("modetest"),
      chainId: 919,
      accounts: getEnvAccounts(),
      gasPrice: 1000252,
    },
    modemain: {
      url: getRpcUrl("modemain"),
      chainId: 34443,
      accounts: getEnvAccounts(),
      gasPrice: 1000252,
    },
