...
export function removeMarket(dataStore, key) {
  ...
  dataStore.removeAddress(
      hashData(["address", "bytes32"], [key, hashString("MARKET_TOKEN")])
      // keccak256(abi.encode(key, MARKET_TOKEN))
  );

  dataStore.removeAddress(
      hashData(["address", "bytes32"], [key, hashString("INDEX_TOKEN")])
      // keccak256(abi.encode(key, INDEX_TOKEN))
  );

  dataStore.removeAddress(
      hashData(["address", "bytes32"], [key, hashString("LONG_TOKEN")])
  );

  dataStore.removeAddress(
      hashData(["address", "bytes32"], [key, hashString("SHORT_TOKEN")])
  );
